...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--speed", type=float, help="vehicle speed in km/h (default: random between 50 and 100)")
    parser.add_argument(
        "--vehicle", choices=VEHICLE_CLASSES, default=DEFAULT_VEHICLE_CLASS.key,
        help=f"vehicle class, sets the toll rate (default: {DEFAULT_VEHICLE_CLASS.key})",
    )
    parser.add_argument("--rate", type=float, help="override the toll rate in INR per km")
...
def run_cli(args: argparse.Namespace) -> None:
    ...
    result = simulate_default_trip(args.speed, VEHICLE_CLASSES[args.vehicle], args.rate, rng)
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if args.cli:
        ...
        run_app(args.speed, VEHICLE_CLASSES[args.vehicle], args.rate, args.map_file)
